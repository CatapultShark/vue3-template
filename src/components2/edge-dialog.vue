...
const form = reactive({
  form: '开始',
  to: '结束',
  group: '从MySql中查询数据—>对json字符串进行处理—>web接口暴露OpenApi',
...
const initData = () => {
  sourceNode.properties.databaseType &&
    (form.form = sourceNode.properties.databaseType);
  targetNode.properties.databaseType &&
    (form.to = targetNode.properties.databaseType);
};
